...
export class PhinaGamepad {
  ...
  constructor(index) {
    ...
    this._prevSticks = range.call([], 0, 2).map(function () {
      return new Vector2(0, 0);
    });
    this.id = null;
   ...
   * @returns {boolean} 結果
   */
  getStickTilt(stickId, threshold) {
    stickId = stickId || 0;
    threshold = threshold || PhinaGamepad.STICK_TILT_THRESHOLD;
    const prev = this._prevSticks ? this._prevSticks[stickId] : new Vector2(0, 0);
    const current = this.sticks ? this.sticks[stickId] : new Vector2(0, 0);
   ...
   * @returns {boolean} 結果
   */
  getStickStraight(stickId, threshold) {
    stickId = stickId || 0;
    threshold = threshold || PhinaGamepad.STICK_TILT_THRESHOLD;
    const prev = this._prevSticks ? this._prevSticks[stickId] : new Vector2(0, 0);
    const current = this.sticks ? this.sticks[stickId] : new Vector2(0, 0);
  ...
  _updateStick(value, stickId, axisName) {
    if (this.sticks[stickId] === undefined) {
      ...
      this._prevSticks[stickId] = new Vector2(0, 0);
    }
   ...
   * @type {number}
   */
  static STICK_TILT_THRESHOLD = 0.5
}
